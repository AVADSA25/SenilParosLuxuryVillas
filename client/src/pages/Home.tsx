...
const PARALLAX_BG = "https://i.imgur.com/4WnZUCv.jpeg";
...
function SectionCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`mx-3 md:mx-6 mb-4 md:mb-6 rounded-2xl overflow-hidden shadow-sm ${className}`}>
      {children}
    </div>
  );
}
...
export default function Home() {
  return (
    <div className="min-h-screen relative">
      ...
      {/* Fixed parallax background behind all sections */}
      <div
        className="fixed inset-0 -z-10 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: `url(${PARALLAX_BG})` }}
        aria-hidden="true"
      />
      <div className="fixed inset-0 -z-10 bg-graphite/50" aria-hidden="true" />

      {/* Hero — full width, no card treatment */}
      <Hero />
      <KeyFactsBand />

      {/* All sections as floating cards over the parallax background */}
        ...
        <SectionCard>
          <section className="py-24 bg-background" id="villas" data-testid="section-villas">
            <div className="max-w-[1200px] mx-auto px-8">
              <h2 className="font-serif text-4xl md:text-5xl font-semibold mb-6 text-center" data-testid="text-villas-title">
                The Villas
              </h2>
